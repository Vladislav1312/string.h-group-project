...
#define MAX_ERR 133
  ...
  char buf[1024];
  char *res;
  static char unknown_error[1024] = {'\0'};
  if (errnum < 0 || errnum > MAX_ERR) {
    s21_convert_reverse(errnum, buf);

#if defined __APPLE__
    s21_strcpy(unknown_error, "Unknown error: ");
#elif defined __linux__
    s21_strcpy(unknown_error, "Unknown error ");
#endif
    res = s21_strncat(unknown_error, buf, s21_strlen(unknown_error));
  } else {
    res = (char *)error_list[errnum];
  }
...
void s21_convert_reverse(int n, char *str) {
  ...
  char buf;
  for (int i = 0; i < count / 2; i++) {  // переворачиваем обратно
    buf = str[i];
    str[i] = str[count - 1 - i];
    str[count - 1 - i] = buf;
  }
